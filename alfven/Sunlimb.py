...
#     if r_range is None:
#         r_arc = (r * 1e6 / fits_header['RSUN_REF'] + 1) * fits_header['RSUN_OBS']
#         r_vect = np.array([r_arc-d_r, r_arc, r_arc+d_r])
#     else:
...
#     if r_range is None:
#         polar_map = np.mean(polar_map, axis=0)
#
...
def to_polar(sun_map, deg_range, r_range=None, r=None, **kwargs):
    ...
    # interp_grid_x = fits_header['CRPIX1'] + delta_x_pixel_grid - 1
    # interp_grid_y = fits_header['CRPIX2'] + delta_y_pixel_grid - 1
    interp_grid_x = (fits_header['CRPIX1'] - 1) + delta_x_pixel_grid
    interp_grid_y = (2048 - fits_header['CRPIX2']) - delta_y_pixel_grid
...
def space_time_plot(files, deg_range, r, d_time, d_deg=0.01, process=None, ax=None, plot=True, **kwargs):
    space_time_data = []
